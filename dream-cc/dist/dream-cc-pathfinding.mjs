...
var DDLSEdge = class DDLSEdge {
	...
	addFromConstraintSegment(segment) {
		if (this._fromConstraintSegments.indexOf(segment) == -1) this._fromConstraintSegments.push(segment);
	}
...
var DDLSGeom2D = class DDLSGeom2D {
	static locatePosition(x, y, mesh) {
		var closedVertex;
		if (!DDLSGeom2D._randGen) DDLSGeom2D._randGen = new DDLSRandGenerator();
		...
		for (i = 0; i < numSamples; i++) this.__samples.push(mesh.__vertices[DDLSGeom2D._randGen.next()]);
		var currVertex;
		var currVertexPos;
		var distSquared;
		...
		while (faceVisited[currFace.id] || !(objectContainer = this.isInFace(x, y, currFace))) {
			...
			numIter++;
			if (numIter == 50) console.warn("WALK TAKE MORE THAN 50 LOOP", "DDLS");
			if (numIter == 1e3) {
	...
	*/
	static isCircleIntersectingAnyConstraint(x, y, radius, mesh) {
		...
		var face;
		if (loc instanceof DDLSVertex) face = loc.edge.leftFace;
		else if (loc instanceof DDLSEdge) face = loc.leftFace;
		else face = loc;
		...
		while (edgesToCheck.length > 0) {
			...
			if (intersecting) {
				if (edge.isConstrained) return true;
					...
					if (!checkedEdges[edge.id] && !checkedEdges[edge.oppositeEdge.id] && edgesToCheck.indexOf(edge) == -1 && edgesToCheck.indexOf(edge.oppositeEdge) == -1) edgesToCheck.push(edge);
	...
	static getDirection2(x1, y1, x2, y2, x3, y3) {
		...
		if (dot == 0) return 0;
			...
			if (this.distanceSquaredPointToLine(x3, y3, x1, y1, x2, y2) <= DDLSConstants.EPSILON_SQUARED) return 0;
			else return 1;
		} else if (this.distanceSquaredPointToLine(x3, y3, x1, y1, x2, y2) <= DDLSConstants.EPSILON_SQUARED) return 0;
		else return -1;
	}
	...
	static clipSegmentByTriangle(s1x, s1y, s2x, s2y, t1x, t1y, t2x, t2y, t3x, t3y, pResult1 = null, pResult2 = null) {
		...
		var n = 0;
		if (this.intersections2segments(s1x, s1y, s2x, s2y, t1x, t1y, t2x, t2y, pResult1, null)) n++;
		if (n == 0) {
			if (this.intersections2segments(s1x, s1y, s2x, s2y, t2x, t2y, t3x, t3y, pResult1, null)) n++;
		} else if (this.intersections2segments(s1x, s1y, s2x, s2y, t2x, t2y, t3x, t3y, pResult2, null)) {
			if (-DDLSConstants.EPSILON > pResult1.x - pResult2.x || pResult1.x - pResult2.x > DDLSConstants.EPSILON || -DDLSConstants.EPSILON > pResult1.y - pResult2.y || pResult1.y - pResult2.y > DDLSConstants.EPSILON) n++;
		}
		if (n == 0) {
			if (this.intersections2segments(s1x, s1y, s2x, s2y, t3x, t3y, t1x, t1y, pResult1, null)) n++;
		} else if (n == 1) {
	...
	static isDelaunay(edge) {
		...
		return (vOpposite.pos.x - this.__circumcenter.x) * (vOpposite.pos.x - this.__circumcenter.x) + (vOpposite.pos.y - this.__circumcenter.y) * (vOpposite.pos.y - this.__circumcenter.y) >= squaredRadius;
	}
	static getCircumcenter(x1, y1, x2, y2, x3, y3, result = null) {
		if (!result) result = new DDLSPoint2D();
		var m1 = (x1 + x2) / 2;
		var m2 = (y1 + y2) / 2;
		var m3 = (x1 + x3) / 2;
		var m4 = (y1 + y3) / 2;
	...
	static intersections2segments(s1p1x, s1p1y, s1p2x, s1p2y, s2p1x, s2p1y, s2p2x, s2p2y, posIntersection = null, paramIntersection = null, infiniteLineMode = false) {
		...
		if (result) {
			...
			if (paramIntersection) paramIntersection.push(t1, t2);
		}
		return result;
	}
	...
	static isConvex(edge) {
		...
		if (this.getRelativePosition(vRight.pos.x, vRight.pos.y, eLeft) != -1) result = false;
		else {
	...
	static intersections2Circles(cx1, cy1, r1, cx2, cy2, r2, result = null) {
		...
		if ((cx1 != cx2 || cy1 != cy2) && distRadiusSQRD <= (r1 + r2) * (r1 + r2) && distRadiusSQRD >= (r1 - r2) * (r1 - r2)) {
			...
			if (result) result.push(xFirstPart + xFactor * transcendPart, yFirstPart - yFactor * transcendPart, xFirstPart - xFactor * transcendPart, yFirstPart + yFactor * transcendPart);
			return true;
		} else return false;
	}
	static intersectionsSegmentCircle(p0x, p0y, p1x, p1y, cx, cy, r, result = null) {
		...
		if (delta < 0) return false;
			...
			if (result) result.push(p0x + t0 * (p1x - p0x), p0y + t0 * (p1y - p0y), t0);
			return true;
		} else {
			...
			if (0 <= t1 && t1 <= 1) {
				if (result) result.push(p0x + t1 * (p1x - p0x), p0y + t1 * (p1y - p0y), t1);
				intersecting = true;
			}
			return intersecting;
		}
	...
	static tangentsPointToCircle(px, py, cx, cy, r, result) {
		var c2x = (px + cx) / 2;
		var c2y = (py + cy) / 2;
		var r2 = .5 * Math.sqrt((px - cx) * (px - cx) + (py - cy) * (py - cy));
	...
	static distanceSquaredPointToSegment(px, py, ax, ay, bx, by) {
		...
		if (dotProduct < 0) return (px - ax) * (px - ax) + (py - ay) * (py - ay);
		else if (dotProduct <= 1) return (ax - px) * (ax - px) + (ay - py) * (ay - py) - dotProduct * dotProduct * a_b_squaredLength;
...
var DDLSAStar = class {
	...
	findPath(fromX, fromY, toX, toY, resultListFaces, resultListEdges) {
		...
		} else this.__fromFace = loc instanceof DDLSFace ? loc : null;
		loc = DDLSGeom2D.locatePosition(toX, toY, this._mesh);
		locVertex = loc instanceof DDLSVertex ? loc : null;
		if (locVertex) this.__toFace = locVertex.edge.leftFace;
		else if (locEdge = loc instanceof DDLSEdge ? loc : null) this.__toFace = locEdge.leftFace;
		else this.__toFace = loc instanceof DDLSFace ? loc : null;
		...
		while (true) {
			...
			if (this.__curFace == this.__toFace) break;
			...
			while (innerEdge) {
				...
				if (!this.__closedFaces[neighbourFace.id]) {
					...
					if (!this.__openedFaces[neighbourFace.id]) {
						...
						fillDatas = true;
					} else if (this.__scoreF[neighbourFace.id] > f) fillDatas = true;
					if (fillDatas) {
			...
			this.__sortedOpenedFaces.sort((a, b) => {
				if (this.__scoreF[a.id] == this.__scoreF[b.id]) return 0;
				else if (this.__scoreF[a.id] < this.__scoreF[b.id]) return 1;
				else return -1;
			});
		}
		if (!this.__curFace) return;
	...
	isWalkableByRadius(fromEdge, throughFace, toEdge) {
		...
		if (dot <= 0) {
			...
			if (distSquared >= this._diameterSquared) return true;
			else return false;
		}
		...
		if (adjEdge.isConstrained) {
			...
			if (distSquaredA < this._diameterSquared || distSquaredB < this._diameterSquared) return false;
			else {
				var vFaceToCheck = [];
				var vFaceIsFromEdge = [];
				var facesDone = {};
				...
				while (vFaceToCheck.length > 0) {
					...
					if (!facesDone[nextFaceA.id] && DDLSGeom2D.distanceSquaredVertexToEdge(vC, currEdgeA) < this._diameterSquared) {
						if (currEdgeA.isConstrained) return false;
						else {
					...
					if (!facesDone[nextFaceB.id] && DDLSGeom2D.distanceSquaredVertexToEdge(vC, currEdgeB) < this._diameterSquared) {
						if (currEdgeB.isConstrained) return false;
						else {
...
var DDLSMatrix2D = class DDLSMatrix2D {
	...
	rotate(rad) {
		...
		this._e * cos + this._f * -sin;
		this._e * sin + this._f * cos;
		this._a = this.a;
		this._b = this.b;
		this._c = this.c;
		this._d = this.d;
	...
	concat(matrix) {
		this._a * matrix.a + this._b * matrix.c;
		this._a * matrix.b + this._b * matrix.d;
		this._c * matrix.a + this._d * matrix.c;
		this._c * matrix.b + this._d * matrix.d;
		this._e * matrix.a + this._f * matrix.c + matrix.e;
		this._e * matrix.b + this._f * matrix.d + matrix.f;
		this._a = this.a;
		this._b = this.b;
		this._c = this.c;
		this._d = this.d;
...
var DDLSObject = class DDLSObject {
	...
	get edges() {
		var res = [];
		for (var i = 0; i < this._constraintShape.segments.length; i++) for (var j = 0; j < this._constraintShape.segments[i].edges.length; j++) res.push(this._constraintShape.segments[i].edges[j]);
		return res;
	}
};
DDLSObject.INC = 0;
...
var DDLSFunnel = class {
	constructor() {
		...
		for (var i = 0; i < this._poolPointsSize; i++) this._poolPoints.push(new DDLSPoint2D());
	}
	dispose() {
	...
	set radius(value) {
		...
		if (this._radius == 0) return;
		for (var i = 0; i < this._numSamplesCircle; i++) this._sampleCircle.push(new DDLSPoint2D(this._radius * Math.cos(-2 * Math.PI * i / this._numSamplesCircle), this._radius * Math.sin(-2 * Math.PI * i / this._numSamplesCircle)));
	...
	findPath(fromX, fromY, toX, toY, listFaces, listEdges, resultPath) {
		...
		for (i = 1; i < listEdges.length; i++) {
			...
			if (currEdge.originVertex == fromVertex) currVertex = currEdge.destinationVertex;
				...
				fromVertex = fromFromVertex;
			} else console.log("IMPOSSIBLE TO IDENTIFY THE VERTEX !!!");
		...
		if (!blocked) for (j = funnelLeft.length - 2; j >= 0; j--) {
		...
		if (this.radius > 0) {
			...
			if (pathPoints.length == 2) this.adjustWithTangents(pathPoints[0], false, pathPoints[1], false, pointSides, pointSuccessor, newPath, adjustedPoints);
			else if (pathPoints.length > 2) {
				this.adjustWithTangents(pathPoints[0], false, pathPoints[1], true, pointSides, pointSuccessor, newPath, adjustedPoints);
				if (pathPoints.length > 3) for (i = 1; i <= pathPoints.length - 3; i++) this.adjustWithTangents(pathPoints[i], true, pathPoints[i + 1], true, pointSides, pointSuccessor, newPath, adjustedPoints);
			...
			for (i = newPath.length - 2; i >= 1; i--) {
				...
				while (smoothPoints.length) adjustedPoints.splice(i * 2, 0, smoothPoints.pop());
			}
		} else adjustedPoints = pathPoints;
		for (i = 0; i < adjustedPoints.length; i++) resultPath.push(adjustedPoints[i].clone());
	}
	adjustWithTangents(p1, applyRadiusToP1, p2, applyRadiusToP2, pointSides, pointSuccessor, newPath, adjustedPoints) {
		...
		} else if (DDLSGeom2D.tangentsCrossCircleToCircle(this._radius, p1.x, p1.y, p2.x, p2.y, tangentsResult)) {
			pTangent1 = this.getPoint(tangentsResult[0], tangentsResult[1]);
			pTangent2 = this.getPoint(tangentsResult[4], tangentsResult[5]);
		} else {
			console.log("NO TANGENT, points are too close for radius");
		...
		while (successor != p2) {
			...
			if (distance < this._radiusSquared) {
				...
				return;
			} else successor = pointSuccessor.get(successor.id);
		}
	...
	smoothAngle(prevPoint, pointToSmooth, nextPoint, side, encirclePoints) {
		...
		if ((prevPoint.x - nextPoint.x) * (prevPoint.x - nextPoint.x) + (prevPoint.y - nextPoint.y) * (prevPoint.y - nextPoint.y) <= this._sampleCircleDistanceSquared) return;
		var index = 0;
		var side1;
		var side2;
		var pointInArea;
...
var DDLSPathFinder = class {
	...
	*/
	...
	findPath(startX, startY, toX, toY, resultPath, radius = 0) {
		...
		if (!this._mesh) throw new Error("Mesh missing");
		if (DDLSGeom2D.isCircleIntersectingAnyConstraint(toX, toY, radius, this._mesh)) return false;
		...
		if (this.__listFaces.length == 0) return false;
...
var DDLSGraph = class DDLSGraph {
	...
	dispose() {
		while (this._node) this.deleteNode(this._node);
	}
	get edge() {
		return this._edge;
	}
	...
	deleteNode(node) {
		while (node.outgoingEdge) {
			if (node.outgoingEdge.oppositeEdge) this.deleteEdge(node.outgoingEdge.oppositeEdge);
		...
		while (otherNode) {
			...
			if (incomingEdge) this.deleteEdge(incomingEdge);
	...
	insertEdge(fromNode, toNode) {
		if (fromNode.successorNodes.has(toNode.id)) return null;
...
var DDLSConstraintSegment = class DDLSConstraintSegment {
	...
	addEdge(edge) {
		if (this._edges.indexOf(edge) == -1 && this._edges.indexOf(edge.oppositeEdge) == -1) this._edges.push(edge);
	}
	removeEdge(edge) {
		var index = this._edges.indexOf(edge);
		if (index == -1) index = this._edges.indexOf(edge.oppositeEdge);
...
var DDLSConstraintShape = class DDLSConstraintShape {
	...
	dispose() {
		while (this._segments.length > 0) this._segments.pop().dispose();
		this._segments = null;
	}
};
...
var IteratorFromVertexToOutgoingEdges = class {
	...
	set fromVertex(value) {
		this._fromVertex = value;
		this._nextEdge = this._fromVertex.edge;
		if (this._nextEdge == void 0 || this._nextEdge.isReal == void 0) throw "aaa";
...
var DDLSMesh = class DDLSMesh {
	...
	buildFromRecord(rec) {
		...
		for (var i = 0; i < positions.length; i += 4) this.insertConstraintSegment(Number(positions[i]), Number(positions[i + 1]), Number(positions[i + 2]), Number(positions[i + 3]));
	}
	insertObject(object) {
		if (object.constraintShape) this.deleteObject(object);
	...
	deleteObject(object) {
		if (!object.constraintShape) return;
	...
	updateObjects() {
		...
		for (var i = 0; i < this._objects.length; i++) if (this._objects[i].hasChanged) {
	...
	deleteConstraintShape(shape) {
		if (shape.segments) for (var i = 0; i < shape.segments.length; i++) this.deleteConstraintSegment(shape.segments[i]);
		shape.dispose();
	...
	insertConstraintSegment(x1, y1, x2, y2) {
		...
		var newX2 = x2;
		var newY2 = y2;
		if (x1 > this._width && x2 > this._width || x1 < 0 && x2 < 0 || y1 > this._height && y2 > this._height || y1 < 0 && y2 < 0) return null;
		else {
			var nx = x2 - x1;
			var ny = y2 - y1;
		...
		if (!vertexDown) return null;
		var vertexUp = this.insertVertex(newX2, newY2);
		if (!vertexUp) return null;
		...
		while (true) {
			...
			if (currVertex instanceof DDLSVertex) {
				...
				if (edgeLeft.destinationVertex == vertexUp) {
					...
					tempEdgeDownUp.originVertex = vertexDown;
					currObjet = vertexDown;
				} else if (DDLSGeom2D.intersections2edges(edgeLeft, tempEdgeDownUp, pIntersect)) {
	...
	deleteConstraintSegment(segment) {
		...
		vertex.removeFromConstraintSegment(segment);
		vertexToDelete.push(vertex);
		for (i = 0; i < vertexToDelete.length; i++) this.deleteVertex(vertexToDelete[i]);
		segment.dispose();
	}
	check() {
		for (var i = 0; i < this.__edges.length; i++) if (!this.__edges[i].nextLeftEdge) {
	...
	insertVertex(x, y) {
		...
		if (inVertex) newVertex = inVertex;
		else if (inEdge = inObject instanceof DDLSEdge ? inObject : null) newVertex = this.splitEdge(inEdge, x, y);
		else if (inFace = inObject instanceof DDLSFace ? inObject : null) newVertex = this.splitFace(inFace, x, y);
	...
	splitEdge(edge, x, y) {
		...
		if ((vLeft.pos.x - x) * (vLeft.pos.x - x) + (vLeft.pos.y - y) * (vLeft.pos.y - y) <= DDLSConstants.EPSILON_SQUARED) return vLeft;
		if ((vRight.pos.x - x) * (vRight.pos.x - x) + (vRight.pos.y - y) * (vRight.pos.y - y) <= DDLSConstants.EPSILON_SQUARED) return vRight;
		...
		if (eLeft_Right.isConstrained) {
			...
			for (var i = 0; i < eLeft_Right.fromConstraintSegments.length; i++) {
				...
				if (index != -1) edges.splice(index, 1, eLeft_Center, eCenter_Right);
				else edges.splice(edges.indexOf(eRight_Left), 1, eRight_Center, eCenter_Left);
			}
	...
	deleteVertex(vertex) {
		...
		if (freeOfConstraint) {
			...
			for (i = 0; i < outgoingEdges.length; i++) {
				...
				} else if (!constrainedEdgeA) boundB.push(edge.nextLeftEdge);
				else if (!constrainedEdgeB) boundA.push(edge.nextLeftEdge);
			...
			for (i = 0; i < vertex.fromConstraintSegments.length; i++) {
				...
				if (index != -1) edges.splice(index - 1, 2, edgeA);
				else edges.splice(edges.indexOf(constrainedEdgeB) - 1, 2, edgeB);
			}
		}
		var faceToDelete;
		for (i = 0; i < outgoingEdges.length; i++) {
		...
		if (bound.length < 2) {
			...
			for (i = 2; i < bound.length; i++) {
				...
				if (DDLSGeom2D.getRelativePosition2(vertexC.pos.x, vertexC.pos.y, baseEdge) == 1) {
					...
					for (var j = 2; j < bound.length; j++) if (j != i) {
...
var EdgeMap = class {
	...
	push(value) {
		if (this.content[value.id]) throw new Error("重复添加");
		this.content[value.id] = value;
		this.dataChanged = true;
	}
	splice(edge) {
		this.content[edge.id];
		delete this.content[edge.id];
		this.dataChanged = true;
	}
	get vector() {
		if (!this._vector) this._vector = [];
		if (this.dataChanged) {
			this._vector.length = 0;
			for (var i of this.content) this._vector.push(i);
...
var DDLSUtils = class DDLSUtils {
	...
	*/
	static isCircleIntersectingAnyConstraint(x, y, radius, mesh, result) {
		result = result || [];
		result.length = 0;
		if (x <= 0 || x >= mesh.width || y <= 0 || y >= mesh.height) return null;
		...
		while (edgesToCheck.length > 0) {
			...
			if (intersecting) {
				if (edge.isConstrained) result.push(edge);
				else {
					edge = edge.oppositeEdge.nextLeftEdge;
					if (!DDLSUtils.checkedEdges.has(edge.id) && !DDLSUtils.checkedEdges.has(edge.oppositeEdge.id) && edgesToCheck.indexOf(edge) == -1 && edgesToCheck.indexOf(edge.oppositeEdge) == -1) edgesToCheck.push(edge);
					edge = edge.nextLeftEdge;
					if (!DDLSUtils.checkedEdges.has(edge.id) && !DDLSUtils.checkedEdges.has(edge.oppositeEdge.id) && edgesToCheck.indexOf(edge) == -1 && edgesToCheck.indexOf(edge.oppositeEdge) == -1) edgesToCheck.push(edge);
				}
			}
	...
	*/
	static pathFormatting(path, result, checkDup = false) {
		...
		for (let index = 0; index < path.length; index += 2) {
			...
			if (index > 0) result.push(path[index], path[index + 1]);
		}
...
var Polygon = class Polygon {
	...
	*  'x' and 'y' coordinates. */
	addVertices(...args) {
		...
		if (numArgs > 0) {
			if (typeof args[0] == "number") for (i = 0; i < numArgs; ++i) {
				const value = args[i];
	...
	contains(x, y) {
		...
		for (i = 0; i < this.numVertices; ++i) {
			...
			if ((iy < y && jy >= y || jy < y && iy >= y) && (ix <= x || jx <= x)) oddNodes ^= Number(ix + (y - iy) / (jy - iy) * (jx - ix) < x);
			j = i;
		}
		return oddNodes != 0;
	}
	...
	toString() {
		...
		for (let i = 0; i < numPoints; ++i) result += "  [Vertex " + i + ": x=" + this._coords[i * 2].toFixed(1) + ", y=" + this._coords[i * 2 + 1].toFixed(1) + "]" + (i == numPoints - 1 ? "\n" : ",\n");
		return result + "]";
	}
	...
	get isSimple() {
		...
		for (let i = 0; i < numCoords; i += 2) {
			...
			for (let j = i + 4; j < endJ; j += 2) {
				...
				if (Polygon.areVectorsIntersecting(ax, ay, bx, by, cx, cy, dx, dy)) return false;
			}
		}
		return true;
	}
	...
	*  points inside the polygon lies inside it, as well. */
	get isConvex() {
		let numCoords = this._coords.length;
		if (numCoords < 6) return true;
		else for (let i = 0; i < numCoords; i += 2) if (!Polygon.isConvexTriangle(this._coords[i], this._coords[i + 1], this._coords[(i + 2) % numCoords], this._coords[(i + 3) % numCoords], this._coords[(i + 4) % numCoords], this._coords[(i + 5) % numCoords])) return false;
		return true;
	}
	...
	get area() {
		let area = 0;
		let numCoords = this._coords.length;
		if (numCoords >= 6) for (let i = 0; i < numCoords; i += 2) {
	...
	*  value will fill up the path with zeros. */
	...
	set numVertices(value) {
		...
		if (oldLength < value) for (let i = oldLength; i < value; ++i) this._coords[i * 2] = this._coords[i * 2 + 1] = 0;
	}
...
var DDLSSimpleView = class extends Node {
	...
	*/
	drawPoint(x, y, radius, color, cleanBefore) {
		if (cleanBefore) this.entities.clear();
	...
	*/
	drawPathByPoints(path, cleanBefore = true) {
		if (cleanBefore) this.paths.clear();
	...
	*/
	drawMesh(mesh) {
		...
		while (vertex = iterVertices.next()) {
			...
			if (!this.vertexIsInsideAABB(vertex, mesh)) continue;
			this.vertices.fillColor = this.colorVertices;
			this.vertices.circle(vertex.pos.x, this.getY(vertex.pos.y), .5);
			constraintsFrist = edgesFrist = true;
			iterEdges.fromVertex = vertex;
			while (incomingEdge = iterEdges.next()) if (!dictVerticesDone.has(incomingEdge.originVertex.id)) {
				if (incomingEdge.isConstrained) {
					...
					} else this.constraints.lineTo(incomingEdge.destinationVertex.pos.x, this.getY(incomingEdge.destinationVertex.pos.y));
					...
					if (edgesFrist) {
						...
						edgesFrist = false;
					} else this.edges.lineTo(incomingEdge.destinationVertex.pos.x, this.getY(incomingEdge.destinationVertex.pos.y));
...
var Graph = class extends EventDispatcher {
	constructor(options) {
		...
		this.__multiEdges = {};
		if (this.__options.multigraph === void 0) this.__options.multigraph = false;
		this.createLink = this.__options.multigraph ? this.__createUniqueLink : this.__createSingleLink;
	}
	addNode(id, data) {
		if (id == void 0) throw new Error("Invalid node identifier.");
	...
	addLink(fromId, toId, data) {
		...
		if (this.__links.has(link_id)) return this.__links.get(link_id);
		...
		if (fromId !== toId) this.addLinkToNode(toNode, link);
	...
	getLink(fromId, toId) {
		...
		for (let index = 0; index < node.links.length; index++) {
			...
			if (link.fromId === fromId && link.toId === toId) return link;
		}
		return null;
	}
	removeLink(link) {
		if (!link) return false;
		if (this.__links.has(link.id)) return false;
	...
	__createUniqueLink(fromId, toId, data) {
		...
		if (isMultiEdge || this.getLink(fromId, toId)) {
			if (!isMultiEdge) this.__multiEdges[linkId] = 0;
	...
	__createSingleLink(fromId, toId, data) {
		return new GraphLink(fromId, toId, data, this.__makeLinkId(fromId, toId));
	}
	get nodeCount() {
	...
	*/
	...
	clear() {
		while (this.__nodes.size > 0) this.removeNode(this.__nodes.keys().next().value);
		this.__nodes.clear();
		this.__links.clear();
		this.__multiEdges = {};
	}
	forEachNode(callback) {
		...
		while (!next_value.done) {
			if (callback(next_value.value)) return true;
	...
	forEachLinkedNode(nodeId, callback, oriented) {
		...
		if (oriented) return this.forEachOrientedLink(node.links, nodeId, callback);
		else return this.forEachNonOrientedLink(node.links, nodeId, callback);
	}
	forEachNonOrientedLink(links, nodeId, callback) {
		for (let index = 0; index < links.length; index++) {
			const link = links[index];
			const lined_node_id = link.fromId === nodeId ? link.toId : link.fromId;
			if (callback(this.getNode(lined_node_id), link)) return true;
		}
	}
	forEachOrientedLink(links, nodeId, callback) {
		for (var i = 0; i < links.length; ++i) {
			...
			if (link.fromId === nodeId) {
				if (callback(this.getNode(link.toId), link)) return true;
			}
		}
	}
...
var DefaultSettings = class {
	...
	*/
	static get heuristic() {
		if (!this.__heuristic) return this.defaultHeuristic;
	...
	*/
	static get distance() {
		if (!this.__distance) return this.defaultDistance;
	...
	*/
	static get compareFScore() {
		if (!this.__compareFScore) return this.defaultCompareFScore;
	...
	static get setHeapIndex() {
		if (!this.__setHeapIndex) return this.defaultSetHeapIndex;
...
var NodeHeap = class {
	constructor(options) {
		...
		if (this.__length > 0) for (let index = this.__length >> 1; index >= 0; index--) this._down(index);
		if (options.setNodeId) for (let index = 0; index < this.__length; index++) this.setNodeId(this.__data[index], index);
	}
	push(item) {
...
var AStar = class {
	...
	find(fromId, toId) {
		...
		while (this.__openSet.length > 0) {
			...
			if (this.goalReached(cameFrom, to)) return this.reconstructPath(cameFrom);
