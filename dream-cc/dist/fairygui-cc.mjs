...
let AutoSizeType = /* @__PURE__ */ function(AutoSizeType) {
...
let VertAlignType = /* @__PURE__ */ function(VertAlignType) {
...
let LoaderFillType = /* @__PURE__ */ function(LoaderFillType) {
...
let ListLayoutType = /* @__PURE__ */ function(ListLayoutType) {
...
let ListSelectionMode = /* @__PURE__ */ function(ListSelectionMode) {
...
let OverflowType = /* @__PURE__ */ function(OverflowType) {
	OverflowType[OverflowType["Visible"] = 0] = "Visible";
	OverflowType[OverflowType["Hidden"] = 1] = "Hidden";
	OverflowType[OverflowType["Scroll"] = 2] = "Scroll";
	return OverflowType;
}({});
let PackageItemType = /* @__PURE__ */ function(PackageItemType) {
...
let ObjectType = /* @__PURE__ */ function(ObjectType) {
...
let ProgressTitleType = /* @__PURE__ */ function(ProgressTitleType) {
...
let ScrollBarDisplayType = /* @__PURE__ */ function(ScrollBarDisplayType) {
...
let ScrollType = /* @__PURE__ */ function(ScrollType) {
	ScrollType[ScrollType["Horizontal"] = 0] = "Horizontal";
	ScrollType[ScrollType["Vertical"] = 1] = "Vertical";
	ScrollType[ScrollType["Both"] = 2] = "Both";
	return ScrollType;
}({});
let FlipType = /* @__PURE__ */ function(FlipType) {
...
let ChildrenRenderOrder = /* @__PURE__ */ function(ChildrenRenderOrder) {
...
let GroupLayoutType = /* @__PURE__ */ function(GroupLayoutType) {
...
let PopupDirection = /* @__PURE__ */ function(PopupDirection) {
...
let FillMethod = /* @__PURE__ */ function(FillMethod) {
...
let FillOrigin = /* @__PURE__ */ function(FillOrigin) {
...
let ObjectPropID = /* @__PURE__ */ function(ObjectPropID) {
...
var BlendModeUtils = class {
	static apply(node, blendMode) {
		let f = factors[blendMode];
		node.getComponentsInChildren(UIRenderer).forEach((element) => {
...
function borrowEvent(type, bubbles) {
	...
	if (eventPool.length) {
		...
		evt.type = type;
		evt.bubbles = bubbles;
	} else evt = new FGUIEvent(type, bubbles);
	return evt;
}
...
var GearBase = class GearBase {
	...
	setup(buffer) {
		...
		if ("pages" in this) this.pages = buffer.readSArray(cnt);
		else {
			for (i = 0; i < cnt; i++) {
				page = buffer.readS();
...
var GearAnimation = class extends GearBase {
	init() {
		this._default = {
			playing: this._owner.getProp(4),
			frame: this._owner.getProp(5)
		};
		this._storage = {};
	}
	addStatus(pageId, buffer) {
	...
	apply() {
		...
		this._owner.setProp(4, gv.playing);
		this._owner.setProp(5, gv.frame);
	...
	updateState() {
		...
		gv.playing = this._owner.getProp(4);
		gv.frame = this._owner.getProp(5);
	}
};
//#endregion
...
var GearColor = class extends GearBase {
	init() {
		this._default = {
			color: this._owner.getProp(2),
			strokeColor: this._owner.getProp(3)
		};
		this._storage = {};
	}
	addStatus(pageId, buffer) {
	...
	apply() {
		this._owner._gearLocked = true;
		var gv = this._storage[this._controller.selectedPageId] || this._default;
		this._owner.setProp(2, gv.color);
		this._owner.setProp(3, gv.strokeColor);
		this._owner._gearLocked = false;
	}
	updateState() {
		var gv = this._storage[this._controller.selectedPageId];
		...
		gv.color = this._owner.getProp(2);
		gv.strokeColor = this._owner.getProp(3);
	}
};
//#endregion
...
var GearFontSize = class extends GearBase {
	...
	init() {
		this._default = this._owner.getProp(8);
		this._storage = {};
	}
	addStatus(pageId, buffer) {
	...
	apply() {
		...
		if (data !== void 0) this._owner.setProp(8, data);
		else this._owner.setProp(8, this._default);
		this._owner._gearLocked = false;
	}
	updateState() {
		this._storage[this._controller.selectedPageId] = this._owner.getProp(8);
	}
};
//#endregion
...
var GearIcon = class extends GearBase {
	...
	apply() {
		this._owner._gearLocked = true;
		var data = this._storage[this._controller.selectedPageId];
		if (data !== void 0) this._owner.icon = data;
...
function evaluateEase(easeType, time, duration, overshootOrAmplitude, period) {
	switch (easeType) {
		case 0: return time / duration;
		case 1: return -Math.cos(time / duration * _PiOver2) + 1;
		case 2: return Math.sin(time / duration * _PiOver2);
		case 3: return -.5 * (Math.cos(Math.PI * time / duration) - 1);
		case 4: return (time /= duration) * time;
		case 5: return -(time /= duration) * (time - 2);
		case 6:
			if ((time /= duration * .5) < 1) return .5 * time * time;
			return -.5 * (--time * (time - 2) - 1);
		case 7: return (time /= duration) * time * time;
		case 8: return (time = time / duration - 1) * time * time + 1;
		case 9:
			if ((time /= duration * .5) < 1) return .5 * time * time * time;
			return .5 * ((time -= 2) * time * time + 2);
		case 10: return (time /= duration) * time * time * time;
		case 11: return -((time = time / duration - 1) * time * time * time - 1);
		case 12:
			if ((time /= duration * .5) < 1) return .5 * time * time * time * time;
			return -.5 * ((time -= 2) * time * time * time - 2);
		case 13: return (time /= duration) * time * time * time * time;
		case 14: return (time = time / duration - 1) * time * time * time * time + 1;
		case 15:
			if ((time /= duration * .5) < 1) return .5 * time * time * time * time * time;
			return .5 * ((time -= 2) * time * time * time * time + 2);
		case 16: return time == 0 ? 0 : Math.pow(2, 10 * (time / duration - 1));
		case 17:
		...
		case 19: return -(Math.sqrt(1 - (time /= duration) * time) - 1);
		case 20: return Math.sqrt(1 - (time = time / duration - 1) * time);
		case 21:
		...
		case 25: return (time /= duration) * time * ((overshootOrAmplitude + 1) * time - overshootOrAmplitude);
		case 26: return (time = time / duration - 1) * time * ((overshootOrAmplitude + 1) * time + overshootOrAmplitude) + 1;
		case 27:
		...
		case 28: return bounce_easeIn(time, duration);
		case 29: return bounce_easeOut(time, duration);
		case 30: return bounce_easeInOut(time, duration);
...
function bounce_easeOut(time, duration) {
	if ((time /= duration) < 1 / 2.75) return 7.5625 * time * time;
	if (time < 2 / 2.75) return 7.5625 * (time -= 1.5 / 2.75) * time + .75;
	if (time < 2.5 / 2.75) return 7.5625 * (time -= 2.25 / 2.75) * time + .9375;
	return 7.5625 * (time -= 2.625 / 2.75) * time + .984375;
}
function bounce_easeInOut(time, duration) {
	if (time < duration * .5) return bounce_easeIn(time * 2, duration) * .5;
...
var TweenValue = class {
	...
	getField(index) {
		switch (index) {
			...
			default: throw new Error("Index out of bounds: " + index);
		}
	}
	...
	update() {
		...
		if (this._valueSize == 6) {
			...
			for (var i = 0; i < cnt; i++) {
				...
				var f = n1 + (this._endValue.getField(i) - n1) * this._normalizedTime;
		...
		if (this._target && this._propType) {
			if (this._propType instanceof Function) switch (this._valueSize) {
				...
				case 5:
					...
					break;
				case 6: this._propType.call(this._target, this._value.x, this._value.y);
			}
			else if (this._valueSize == 5) this._target[this._propType] = this._value.color;
			else this._target[this._propType] = this._value.x;
		}
		this.callUpdateCallback();
	}
	callStartCallback() {
		if (this._onStart) try {
...
var TweenManager = class TweenManager {
	...
	static getTween(target, propType) {
		...
		for (var i = 0; i < cnt; i++) {
			var tweener = _activeTweens[i];
			if (tweener && tweener.target == target && !tweener._killed && (anyType || tweener._propType == propType)) return tweener;
		}
		return null;
	}
	static update(dt) {
...
var GearText = class extends GearBase {
	...
	apply() {
		this._owner._gearLocked = true;
		var data = this._storage[this._controller.selectedPageId];
		if (data !== void 0) this._owner.text = data;
...
var RelationItem = class {
	...
	add(relationType, usePercent) {
		...
		for (var i = 0; i < length; i++) if (this._defs[i].type == relationType) return;
	...
	remove(relationType) {
		...
		for (var k = 0; k < dc; k++) if (this._defs[k].type == relationType) {
	...
	applyOnSelfResized(dWidth, dHeight, applyPivot) {
		...
		var oy = this._owner.y;
		var length = this._defs.length;
		for (var i = 0; i < length; i++) switch (this._defs[i].type) {
			case 3:
			...
			case 12:
			case 11:
			case 13: this._owner.y -= (1 - (applyPivot ? this._owner.pivotY : 0)) * dHeight;
		}
		if (ox != this._owner.x || oy != this._owner.y) {
			...
			if (this._owner.parent) {
				...
				if (len > 0) for (var i = 0; i < len; ++i) this._owner.parent._transitions[i].updateFromRelations(this._owner.id, ox, oy);
			}
		}
	}
	applyOnXYChanged(info, dx, dy) {
		...
		switch (info.type) {
			...
			case 21:
				...
				break;
			case 22:
			case 23: if (this._owner != this._target.parent) {
				tmp = this._owner.yMin;
				this._owner.height = this._owner._rawHeight + dy;
	...
	applyOnSizeChanged(info) {
		...
		switch (info.type) {
			...
			case 1:
				if (info.percent) this._owner.xMin = pos + (this._owner.xMin - pos) * delta;
				else this._owner.x += delta * (.5 - pivot);
				break;
			case 2:
				if (info.percent) this._owner.xMin = pos + (this._owner.xMin - pos) * delta;
				...
				break;
			case 3:
				if (info.percent) this._owner.xMin = pos + (this._owner.xMin + this._owner._rawWidth * .5 - pos) * delta - this._owner._rawWidth * .5;
				else this._owner.x += delta * (.5 - pivot);
				break;
			case 4:
			...
			case 8:
				if (info.percent) this._owner.yMin = pos + (this._owner.yMin - pos) * delta;
				else this._owner.y += delta * (.5 - pivot);
				break;
			case 9:
				if (info.percent) this._owner.yMin = pos + (this._owner.yMin - pos) * delta;
				...
				break;
			case 10:
				if (info.percent) this._owner.yMin = pos + (this._owner.yMin + this._owner._rawHeight * .5 - pos) * delta - this._owner._rawHeight * .5;
				else this._owner.y += delta * (.5 - pivot);
				break;
			case 11:
	...
	addRefTarget(target) {
		if (target != this._owner.parent) target.on(FGUIEvent.XY_CHANGED, this.__targetXYChanged, this);
...
var Relations = class {
	...
	contains(target) {
		var length = this._items.length;
		for (var i = 0; i < length; i++) if (this._items[i].target == target) return true;
		return false;
	}
	clearFor(target) {
		var cnt = this._items.length;
	...
	clearAll() {
		var length = this._items.length;
		for (var i = 0; i < length; i++) this._items[i].dispose();
	...
	onOwnerSizeChanged(dWidth, dHeight, applyPivot) {
		...
		var length = this._items.length;
		for (var i = 0; i < length; i++) this._items[i].applyOnSelfResized(dWidth, dHeight, applyPivot);
	}
	ensureRelationsSizeCorrect() {
		if (this._items.length == 0) return;
		this.sizeDirty = false;
		var length = this._items.length;
		for (var i = 0; i < length; i++) this._items[i].target.ensureSizeCorrect();
	}
	get empty() {
...
UIConfig.modalLayerColor = new Color(51, 51, 51, 51);
...
UIConfig.frameTimeForAsyncUIConstruction = .002;
UIConfig.linkUnderline = true;
UIConfig.defaultUILayer = Layers.Enum.UI_2D;
let _fontRegistry = {};
function registerFont(name, font, bundle) {
	if (font instanceof Font) _fontRegistry[name] = font;
	else (bundle || resources).load(name, Font, (err, asset) => {
...
var TooltipManager = class {
	...
	*/
	static register(type, value) {
		if (this.impl) this.impl.register(type, value);
	}
	/**
	* 注销
	* @param type
	*/
	static unregister(type) {
		if (this.impl) this.impl.unregister(type);
	}
	/**
	* 显示
	* @param data 
	*/
	static show(data) {
		if (this.impl) this.impl.show(data);
	}
	/**
	* 隐藏
	*/
	static hide() {
		if (this.impl) this.impl.hide();
	}
	static get impl() {
		if (this.__impl == null) this.__impl = Injector.getInject(this.KEY);
		if (this.__impl == null) throw new Error(`${this.KEY} 未注入`);
		return this.__impl;
	}
};
...
var GObject = class GObject {
	...
	set xMin(value) {
		if (this._pivotAsAnchor) this.setPosition(value + this._width * this._uiTrans.anchorX, this._y);
	...
	set yMin(value) {
		if (this._pivotAsAnchor) this.setPosition(this._x, value + this._height * (1 - this._uiTrans.anchorY));
	...
	setSize(wv, hv, ignorePivot) {
		if (this._rawWidth != wv || this._rawHeight != hv) {
			...
			if ((this._uiTrans.anchorX != 0 || this._uiTrans.anchorY != 1) && !this._pivotAsAnchor && !ignorePivot) this.setPosition(this.x - this._uiTrans.anchorX * dWidth, this.y - (1 - this._uiTrans.anchorY) * dHeight);
	...
	on(type, listener, target) {
		if (type == FGUIEvent.DISPLAY || type == FGUIEvent.UNDISPLAY) this._partner._emitDisplayEvents = true;
	...
	hasEventHandler(type, listener, target) {
		return this._node.hasEventListener(type, listener, target);
	}
	get draggable() {
	...
	_hitTest(pt, globalPt) {
		if (pt.x >= 0 && pt.y >= 0 && pt.x < this._width && pt.y < this._height) return this;
		else return null;
	}
	getProp(index) {
		switch (index) {
			case 0: return this.text;
			case 1: return this.icon;
			case 2: return null;
			case 3: return null;
			case 4: return false;
			case 5: return 0;
	...
	onRollOver() {
		TooltipManager.show(this.tooltips);
	}
	onRollOut() {
		TooltipManager.hide();
	}
	...
	dragBegin(touchId) {
		...
		if (touchId == void 0) touchId = Decls$1.GRoot.inst.inputProcessor.getAllTouches()[0];
...
var GObjectPartner = class extends Component {
	...
	onEnable() {
		...
		if (this._emitDisplayEvents) this.node.emit(FGUIEvent.DISPLAY);
	}
	onDisable() {
		this.node["$gobj"].onDisable();
		if (this._emitDisplayEvents) this.node.emit(FGUIEvent.UNDISPLAY);
	}
	update(dt) {
...
var GGroup = class extends GObject {
	...
	setBoundsChangedFlag(positionChangedOnly = false) {
		if (this._updating == 0 && this._parent) {
			...
			if (!this._boundsChanged) {
				...
				if (this._layout != 0) this._partner.callLater(this._ensureBoundsCorrect);
			}
		}
	}
	_ensureBoundsCorrect() {
		GObject.cast(this.node).ensureBoundsCorrect();
	}
	ensureSizeCorrect() {
	...
	ensureBoundsCorrect() {
		...
		else if (this._autoSizeDisabled) this.resizeChildren(0, 0);
		else {
			this.handleLayout();
			this.updateBounds();
		}
	...
	moveChildren(dx, dy) {
		...
		for (i = 0; i < cnt; i++) {
			child = this._parent.getChildAt(i);
			if (child.group == this) child.setPosition(child.x + dx, child.y + dy);
		}
		this._updating &= 2;
	}
	resizeChildren(dw, dh) {
		if (this._layout == 0 || (this._updating & 2) != 0 || this._parent == null) return;
		this._updating |= 2;
		if (this._boundsChanged) {
			this._boundsChanged = false;
...
var GGraph = class extends GObject {
	...
	updateGraph() {
		...
		if (this._type == 1) {
			if (this._cornerRadius) ctx.roundRect(px + ls, -h + py + ls, w - this._lineSize, h - this._lineSize, this._cornerRadius[0]);
			else ctx.rect(px + ls, -h + py + ls, w - this._lineSize, h - this._lineSize);
		} else if (this._type == 2) ctx.ellipse(w / 2 + px, -h / 2 + py, w / 2 - ls, h / 2 - ls);
		else if (this._type == 3) this.drawPath(ctx, this._polygonPoints, px, py);
	...
	getProp(index) {
		if (index == 2) return this.color;
		else return super.getProp(index);
	}
	setProp(index, value) {
		if (index == 2) this.color = value;
...
var Image$1 = class extends Sprite {
	...
	set flip(value) {
		if (this._flip != value) {
			this._flip = value;
			let sx = 1, sy = 1;
			if (this._flip == 1 || this._flip == 3) sx = -1;
			if (this._flip == 2 || this._flip == 3) sy = -1;
			if (sx != 1 || sy != 1) this.node.getComponent(UITransform).setAnchorPoint(.5, .5);
	...
	set fillMethod(value) {
		if (this._fillMethod != value) {
			...
			if (this._fillMethod != 0) {
				...
				if (this._fillMethod <= 3) this.fillType = this._fillMethod - 1;
				else this.fillType = Sprite.FillType.RADIAL;
				this.fillCenter = new Vec2(.5, .5);
				this.setupFill();
			} else this.type = Sprite.Type.SIMPLE;
		}
	}
	get fillOrigin() {
	...
	set fillAmount(value) {
		if (this._fillAmount != value) {
			...
			if (this._fillMethod != 0) {
				if (this._fillClockwise) this.fillRange = -this._fillAmount;
				else this.fillRange = this._fillAmount;
			}
		}
	}
	setupFill() {
		if (this._fillMethod == 1) {
			this._fillClockwise = this._fillOrigin == 3 || this._fillOrigin == 1;
			this.fillStart = this._fillClockwise ? 1 : 0;
		} else if (this._fillMethod == 2) {
			this._fillClockwise = this._fillOrigin == 2 || this._fillOrigin == 0;
			this.fillStart = this._fillClockwise ? 1 : 0;
...
var GImage = class extends GObject {
	...
	constructFromResource() {
		...
		contentItem.load();
		if (contentItem.scale9Grid) this._content.type = Sprite.Type.SLICED;
		else if (contentItem.scaleByTile) this._content.type = Sprite.Type.TILED;
...
var MovieClip = class extends Image$1 {
	...
	set playing(value) {
		if (this._playing != value) this._playing = value;
	}
	get smoothing() {
...
var GMovieClip = class extends GObject {
	...
	getProp(index) {
		switch (index) {
			...
			case 4: return this.playing;
			case 5: return this.frame;
			case 7: return this.timeScale;
...
var PixelHitTest = class {
	...
	hitTest(pt) {
		...
		var pos3 = pos % 8;
		if (pos2 >= 0 && pos2 < this._data.pixels.length) return (this._data.pixels[pos2] >> pos3 & 1) == 1;
		else return false;
	}
};
var PixelHitTestData = class {
	constructor(ba) {
		ba.readInt();
...
var TranslationHelper = class TranslationHelper {
	static loadFromXML(source) {
		...
		var nodes = new DOMParser().parseFromString(source, "text/xml").documentElement.childNodes;
	...
	static translateComponent(item) {
		...
		for (i = 0; i < childCount; i++) {
			...
			if (baseType == 9 && buffer.version >= 2) {
			...
			switch (type) {
				...
				case 12:
					...
					break;
				case 13: if (buffer.seek(curPos, 6) && buffer.readByte() == type) {
					itemCount = buffer.readShort();
					for (j = 0; j < itemCount; j++) {
						nextPos = buffer.readShort();
						nextPos += buffer.position;
...
var ByteBuffer = class ByteBuffer {
	...
	readString(len) {
		if (len == void 0) len = this.readUshort();
		this.validate(len);
		let pos = this._pos;
		if (window["TextDecoder"]) {
			if (!ByteBuffer.TxtDecoder) ByteBuffer.TxtDecoder = new TextDecoder("utf-8");
			this._pos += len;
			return ByteBuffer.TxtDecoder.decode(this._bytes.subarray(pos, pos + len));
		}
		...
		while (pos < max) {
			...
			if (c < 128) {
				if (c != 0) v += f(c);
			} else if (c < 224) v += f((c & 63) << 6 | u[pos++] & 127);
			else if (c < 240) {
				c2 = u[pos++];
...
var UIPackage = class UIPackage {
	...
	static set branch(value) {
		...
		for (var pkgId in _instById) {
			...
			if (pkg._branches) pkg._branchIndex = pkg._branches.indexOf(value);
		}
	}
	static getVar(key) {
		return _vars[key];
	...
	*/
	...
	static loadPackage(...args) {
		...
		bundle.load(path, Asset, onProgress, (err, asset) => {
			...
			if (total > 0) urls.forEach((url, index) => {
	...
	loadPackage(buffer, path) {
		if (buffer.readUint() != 1179080009) throw new Error("FairyGUI: old package format found in '" + path + "'");
		this._path = path;
		...
		if (buffer.seek(indexTablePos, 3)) {
			...
			for (i = 0; i < cnt; i++) {
				...
				nextPos += buffer.position;
				pi = this._itemsById[buffer.readS()];
				if (pi && pi.type == 0) pi.hitTestData = new PixelHitTestData(buffer);
				buffer.position = nextPos;
			}
		}
	}
	...
	getItemAssetByName(resName) {
		var pi = this._itemsByName[resName];
		if (pi == null) throw new Error("Resource not found -" + resName);
		...
		switch (item.type) {
			...
			case 2:
				if (!item.decoded) {
					...
					if (!item.asset) console.log("Resource '" + item.file + "' not found");
					else if (item.type == 4) {
	...
	loadFont(item) {
		...
		for (var i = 0; i < cnt; i++) {
			...
			if (ttf) {
				...
				if (bg.xAdvance == 0) {
					if (xadvance == 0) bg.xAdvance = bg.xOffset + bg.rect.width;
					else bg.xAdvance = xadvance;
				}
			}
			buffer.position = nextPos;
		}
...
var UBBParser = class {
	...
	onTag_URL(tagName, end, attr) {
		if (!end) {
			let ret;
			if (attr != null) ret = "<on click=\"onClickLink\" param=\"" + attr + "\">";
			else ret = "<on click=\"onClickLink\" param=\"" + this.getTagText() + "\">";
	...
	getTagText(remove) {
		...
		var pos2;
		var result = "";
		while ((pos2 = this._text.indexOf("[", pos1)) != -1) if (this._text.charCodeAt(pos2 - 1) == 92) {
...
var GTextField = class extends GObject {
	...
	set font(value) {
		if (this._font != value || !value) {
			...
			if (newFont.startsWith("ui://")) {
				...
				if (pi) newFont = pi.owner.getItemAsset(pi);
				else newFont = UIConfig.defaultFont;
			}
	...
	updateText() {
		...
		if (this._ubbEnabled) text2 = defaultParser.parse(text2, true);
		this._label.string = text2;
	}
	assignFont(label, value) {
		if (value instanceof Font) label.font = value;
		else {
	...
	updateStrokeColor() {
		...
		if (!this._strokeColor) this._strokeColor = new Color();
		if (this._grayed) this._label.outlineColor = toGrayedColor(this._strokeColor);
		else this._label.outlineColor = this._strokeColor;
	}
	updateShadowColor() {
		if (!this._label || !this._label.enableShadow) return;
		if (!this._shadowColor) this._shadowColor = new Color();
		if (this._grayed) this._label.shadowColor = toGrayedColor(this._shadowColor);
		else this._label.shadowColor = this._shadowColor;
	}
	updateFontSize() {
		let font = this._label.font;
		if (font instanceof BitmapFont) {
			let fntConfig = font.fntConfig;
			if (fntConfig.resizable) this._label.fontSize = this._fontSize;
			else this._label.fontSize = fntConfig.fontSize;
	...
	updateOverflow() {
		if (this._autoSize == 1) this._label.overflow = Label.Overflow.NONE;
		else if (this._autoSize == 2) {
			this._label.overflow = Label.Overflow.RESIZE_HEIGHT;
			this._uiTrans.width = this._width;
		} else if (this._autoSize == 3) {
	...
	markSizeChanged() {
		if (this._underConstruct) return;
		if (this._autoSize == 1 || this._autoSize == 2) {
			if (!this._sizeDirty) {
	...
	handleSizeChanged() {
		...
		if (this._autoSize == 0 || this._autoSize == 3) this._uiTrans.setContentSize(this._width, this._height);
		else if (this._autoSize == 2) this._uiTrans.width = this._width;
	}
	handleGrayedChanged() {
		this.updateFontColor();
		this.updateStrokeColor();
	...
	getProp(index) {
		switch (index) {
			case 2: return this.color;
			case 3: return this.strokeColor;
			case 8: return this.fontSize;
			default: return super.getProp(index);
		}
	}
	setProp(index, value) {
...
var RichTextImageAtlas = class extends SpriteAtlas {
	getSpriteFrame(key) {
		...
		if (pi) {
			pi.load();
			if (pi.type == 0) return pi.asset;
			else if (pi.type == 1) return pi.frames[0].texture;
		}
...
var GRichTextField = class extends GTextField {
	...
	replaceHtmlColor(html) {
		...
		while ((match = regex.exec(html)) !== null) {
			const replacement = `<color=${match[2]}>${match[3]}</color>`;
	...
	updateOverflow() {
		if (this._autoSize == 1) this._richText.maxWidth = 0;
		else this._richText.maxWidth = this._width;
	}
	handleSizeChanged() {
		if (this._updatingSize) return;
		if (this._autoSize != 1) this._richText.maxWidth = this._width;
	}
};
//#endregion
...
var InputProcessor = class extends Component {
	...
	addTouchMonitor(touchId, target) {
		...
		if (!ti) return;
		if (ti.touchMonitors.indexOf(target) == -1) ti.touchMonitors.push(target);
	}
	...
	touchBeginHandler(evt) {
		...
		this.setBegin(ti);
		if (this._touchListener) this._touchListener.setSwallowTouches(ti.target != this._owner);
		else {
			let e = evt;
	...
	touchEndHandler(evt) {
		...
		if (sys.isMobile) this.handleRollOver(ti, null);
		else this.handleRollOver(ti, ti.target);
		ti.target = null;
		ti.touchId = -1;
		ti.button = -1;
	}
	...
	getInfo(touchId, createIfNotExisits) {
		if (createIfNotExisits === void 0) createIfNotExisits = true;
		let ret = null;
		let cnt = this._touches.length;
		for (let i = 0; i < cnt; i++) {
			let ti = this._touches[i];
			if (ti.touchId == touchId) return ti;
			else if (ti.touchId == -1) ret = ti;
		}
		if (!ret) {
	...
	setEnd(ti) {
		ti.began = false;
		let now = game.totalTime / 1e3;
		if (now - ti.lastClickTime < .45) {
			if (ti.clickCount == 2) ti.clickCount = 1;
			else ti.clickCount++;
	...
	clickTest(ti) {
		...
		obj = ti.target;
		while (obj) {
			if (ti.downTargets.indexOf(obj) != -1 && obj.node && obj.node.activeInHierarchy) break;
			obj = obj.findParent();
		}
		return obj;
	}
...
var Controller = class extends EventTarget {
	...
	set oppositePageId(val) {
		if (this._pageIds.indexOf(val) > 0) this.selectedIndex = 0;
	...
	runActions() {
		if (this._actions) {
			...
			for (var i = 0; i < cnt; i++) this._actions[i].run(this, this.previousPageId, this.selectedPageId);
		}
	}
	setup(buffer) {
		...
		var homePageIndex = 0;
		if (buffer.version >= 2) switch (buffer.readByte()) {
			case 1:
...
function createAction(type) {
	switch (type) {
		case 0: return new PlayTransitionAction();
		case 1: return new ChangePageAction();
	}
	return null;
}
//#endregion
...
var ScrollPane = class ScrollPane extends Component {
	...
	setup(buffer) {
		...
		if (scrollBarDisplay == 0) scrollBarDisplay = UIConfig.defaultScrollBarDisplay;
		if (scrollBarDisplay != 3) {
			...
			if (scrollBarDisplay == 2) this._scrollBarDisplayAuto = true;
		...
		if (headerRes) {
			...
			if (this._header == null) throw new Error("cannot create scrollPane header from " + headerRes);
			else this._maskContainer.insertChild(this._header.node, 0);
		}
		if (footerRes) {
			this._footer = UIPackage.createObjectFromURL(footerRes);
			if (this._footer == null) throw new Error("cannot create scrollPane footer from " + footerRes);
			else this._maskContainer.insertChild(this._footer.node, 0);
		}
	...
	handleControllerChanged(c) {
		if (this._pageController == c) {
			if (this._scrollType == 0) this.setCurrentPageX(c.selectedIndex, true);
	...
	updatePageController() {
		if (this._pageController && !this._pageController.changing) {
			var index;
			if (this._scrollType == 0) index = this.currentPageX;
	...
	adjustMaskContainer() {
		...
		const o = this._owner;
		if (this._dontClipMargin) this._maskContainerUITrans.setAnchorPoint((o.margin.left + o._alignOffset.x) / o.width, 1 - (o.margin.top + o._alignOffset.y) / o.height);
		else this._maskContainerUITrans.setAnchorPoint(o._alignOffset.x / this._viewSize.x, 1 - o._alignOffset.y / this._viewSize.y);
		if (o._customMask) this._maskContainer.setPosition(mx + o._alignOffset.x, -o._alignOffset.y);
		else this._maskContainer.setPosition(o._pivotCorrectX + mx + o._alignOffset.x, o._pivotCorrectY - o._alignOffset.y);
	}
	...
	handleSizeChanged() {
		...
		if (this._scrollType == 0 || this._scrollType == 2) this._overlapSize.x = Math.ceil(Math.max(0, this._contentSize.x - this._viewSize.x));
		else this._overlapSize.x = 0;
		if (this._scrollType == 1 || this._scrollType == 2) this._overlapSize.y = Math.ceil(Math.max(0, this._contentSize.y - this._viewSize.y));
		...
		if (this._refreshBarAxis == "x") this._container.setPosition(math.clamp(this._container.position.x, -max, this._headerLockedSize), -math.clamp(-this._container.position.y, -this._overlapSize.y, 0));
		else this._container.setPosition(math.clamp(this._container.position.x, -this._overlapSize.x, 0), -math.clamp(-this._container.position.y, -max, this._headerLockedSize));
		if (this._header) {
	...
	onTouchBegin(evt) {
		...
		this._lastMoveTime = game.totalTime / 1e3;
	}
	onTouchMove(evt) {
		...
		if (sv) {
			if (newPosY > 0) {
				if (!this._bouncebackEffect) this._container.setPosition(this._container.position.x, 0);
				else if (this._header && this._header.maxHeight != 0) this._container.setPosition(this._container.position.x, -Math.floor(Math.min(newPosY * .5, this._header.maxHeight)));
				else this._container.setPosition(this._container.position.x, -Math.floor(Math.min(newPosY * .5, this._viewSize.y * PULL_RATIO)));
			} else if (newPosY < -this._overlapSize.y) {
				if (!this._bouncebackEffect) this._container.setPosition(this._container.position.x, this._overlapSize.y);
				else if (this._footer && this._footer.maxHeight > 0) this._container.setPosition(this._container.position.x, -Math.floor(Math.max((newPosY + this._overlapSize.y) * .5, -this._footer.maxHeight) - this._overlapSize.y));
				else this._container.setPosition(this._container.position.x, -Math.floor(Math.max((newPosY + this._overlapSize.y) * .5, -this._viewSize.y * PULL_RATIO) - this._overlapSize.y));
			} else this._container.setPosition(this._container.position.x, -newPosY);
		}
		if (sh) {
			if (newPosX > 0) {
				if (!this._bouncebackEffect) this._container.setPosition(0, this._container.position.y);
				else if (this._header && this._header.maxWidth != 0) this._container.setPosition(Math.floor(Math.min(newPosX * .5, this._header.maxWidth)), this._container.position.y);
				else this._container.setPosition(Math.floor(Math.min(newPosX * .5, this._viewSize.x * PULL_RATIO)), this._container.position.y);
			} else if (newPosX < 0 - this._overlapSize.x) {
				if (!this._bouncebackEffect) this._container.setPosition(-this._overlapSize.x, this._container.position.y);
				else if (this._footer && this._footer.maxWidth > 0) this._container.setPosition(Math.floor(Math.max((newPosX + this._overlapSize.x) * .5, -this._footer.maxWidth) - this._overlapSize.x), this._container.position.y);
				else this._container.setPosition(Math.floor(Math.max((newPosX + this._overlapSize.x) * .5, -this._viewSize.x * PULL_RATIO) - this._overlapSize.x), this._container.position.y);
			} else this._container.setPosition(newPosX, this._container.position.y);
		}
		var now = game.totalTime / 1e3;
	...
	onTouchEnd(evt) {
		...
		if (flag) {
			...
			if (!this._inertiaDisabled) {
				var elapsed = (game.totalTime / 1e3 - this._lastMoveTime) * 60 - 1;
				if (elapsed > 1) {
					var factor = Math.pow(.833, elapsed);
					this._velocity.x = this._velocity.x * factor;
					this._velocity.y = this._velocity.y * factor;
	...
	onMouseWheel(evt) {
		...
		if (this._overlapSize.x > 0 && this._overlapSize.y == 0) {
			if (this._pageMode) this.setPosX(this._xPos + this._pageSize.x * delta, false);
			else this.setPosX(this._xPos + this._mouseWheelStep * delta, false);
		} else if (this._pageMode) this.setPosY(this._yPos + this._pageSize.y * delta, false);
	...
	updateScrollBarVisible() {
		if (this._vtScrollBar) {
			if (this._viewSize.y <= this._vtScrollBar.minSize || this._vScrollNone) this._vtScrollBar.node.active = false;
			else this.updateScrollBarVisible2(this._vtScrollBar);
		}
		if (this._hzScrollBar) {
			if (this._viewSize.x <= this._hzScrollBar.minSize || this._hScrollNone) this._hzScrollBar.node.active = false;
	...
	updateScrollBarVisible2(bar) {
		...
		if (this._scrollBarDisplayAuto && !this._hover && this._tweening == 0 && !this._dragged && !bar.gripDragging) {
			if (bar.node.active) GTween.to(1, 0, .5).setDelay(.5).onComplete(this.__barTweenComplete, this).setTarget(bar, "alpha");
		} else {
			bar.alpha = 1;
	...
	loopCheckingNewPos(value, axis) {
		...
		if (changed) {
			if (axis == "x") this._container.setPosition(-Math.floor(pos), this._container.position.y);
			else this._container.setPosition(this._container.position.x, Math.floor(pos));
		}
		return value;
	}
	alignPosition(pos, inertialScrolling) {
		...
		if (this._snappingPolicy == 1) {
			if (this._owner.numChildren > 0) {
				...
				ax = Math.floor(this._viewSize.x * .5 - obj.width * .5);
				ay = Math.floor(this._viewSize.y * .5 - obj.height * .5);
			}
	...
	alignByPage(pos, axis, inertialScrolling) {
		...
		if (pos > 0) page = 0;
			...
			} else if (delta > testPageSize * (change < 0 ? .3 : .7)) page++;
	...
	updateTargetAndDuration2(pos, axis) {
		...
		if (pos > 0) pos = 0;
			...
			} else if (v2 > 1e3) ratio = Math.pow((v2 - 1e3) / 1e3, 2);
			if (ratio != 0) {
				if (ratio > 1) ratio = 1;
				v2 *= ratio;
				v *= ratio;
	...
	checkRefreshBar() {
		...
		if (this._header) {
			...
			} else this._header.node.active = false;
		}
		if (this._footer) {
			var max = this._overlapSize[this._refreshBarAxis];
			...
			} else this._footer.node.active = false;
		}
	}
	update(dt) {
...
var GPath = class {
	...
	create(points) {
		...
		var splinePoints = [];
		var prev = points[0];
		if (prev.curveType == 0) splinePoints.push(new Vec2(prev.x, prev.y));
		for (var i = 1; i < cnt; i++) {
	...
	createSplineSegment(splinePoints) {
		...
		seg.length = 0;
		for (var i = 1; i < cnt; i++) seg.length += distance(splinePoints[i - 1].x, splinePoints[i - 1].y, splinePoints[i].x, splinePoints[i].y);
		this._fullLength += seg.length;
		this._segments.push(seg);
	...
	getPointAt(t, result) {
		...
		if (t == 1) {
			...
			if (seg.type == 3) {
				...
				return result;
			} else if (seg.type == 1 || seg.type == 2) return this.onBezierCurve(seg.ptStart, seg.ptCount, t, result);
		...
		for (var i = 0; i < cnt; i++) {
			...
			if (len < 0) {
				...
				if (seg.type == 3) {
					result.x = math.lerp(this._points[seg.ptStart].x, this._points[seg.ptStart + 1].x, t);
					result.y = math.lerp(this._points[seg.ptStart].y, this._points[seg.ptStart + 1].y, t);
				} else if (seg.type == 1 || seg.type == 2) result = this.onBezierCurve(seg.ptStart, seg.ptCount, t, result);
	...
	getPointsInSegment(segmentIndex, t0, t1, points, ts, pointDensity) {
		...
		if (seg.type == 3) {
			...
			var func;
			if (seg.type == 1 || seg.type == 2) func = this.onBezierCurve;
	...
	onCRSplineCurve(ptStart, ptCount, t, result) {
		...
		var adjustedT = t == 1 ? 1 : math.repeat(t * (ptCount - 4), 1);
...
var Transition = class {
	...
	setAutoPlay(value, times, delay) {
		...
		if (this._autoPlay != value) {
			...
			} else if (!this._owner.onStage) this.stop(false, true);
		}
	}
	...
	stop(setToComplete, processCallback) {
		...
		GTween.kill(this);
		var cnt = this._items.length;
		if (this._reversed) for (var i = cnt - 1; i >= 0; i--) {
			var item = this._items[i];
		...
		if (processCallback && func != null) func();
	}
	...
	setPaused(paused) {
		...
		for (var i = 0; i < cnt; i++) {
			...
			if (item.type == 10) {
				...
				if (paused) {
					item.value.flag = item.target.getProp(4);
					item.target.setProp(4, false);
				} else item.target.setProp(4, item.value.flag);
			}
	...
	setValue(label, ...args) {
		...
		for (var i = 0; i < cnt; i++) {
			...
			} else if (item.tweenConfig && item.tweenConfig.endLabel == label) value = item.tweenConfig.endValue;
			else continue;
			switch (item.type) {
				case 0:
				case 1:
			...
			if (this._playing) {
				...
				for (var i = 0; i < cnt; i++) {
					...
					if (item.tweener) item.tweener.setTimeScale(value);
						...
						if (item.target) item.target.setProp(7, value);
					}
				}
			}
		}
	...
	internalPlay() {
		...
		var i;
		if (!this._reversed) for (i = 0; i < cnt; i++) {
			item = this._items[i];
			if (item.target == null) continue;
			...
			if (this._endTime == -1 || time <= this._endTime) {
				...
				switch (item.type) {
					...
					case 6:
						...
						break;
					case 12: item.tweener = GTween.to4(startValue.f1, startValue.f2, startValue.f3, startValue.f4, endValue.f1, endValue.f2, endValue.f3, endValue.f4, item.tweenConfig.duration);
				}
	...
	skipAnimations() {
		...
		for (var i = 0; i < cnt; i++) {
			item = this._items[i];
			if (item.type != 7 || item.time > this._startTime) continue;
			value = item.value;
			if (value.flag) continue;
			target = item.target;
			frame = target.getProp(5);
			playStartTime = target.getProp(4) ? 0 : -1;
			playTotalTime = 0;
			for (var j = i; j < cnt; j++) {
				item = this._items[j];
				if (item.type != 7 || item.target != target || item.time > this._startTime) continue;
				value = item.value;
				value.flag = true;
				if (value.frame != -1) {
					frame = value.frame;
			...
			target.setProp(4, playStartTime >= 0);
			target.setProp(5, frame);
			if (playTotalTime > 0) target.setProp(6, playTotalTime);
		}
	}
		...
		if (item.type == 0 || item.type == 1) {
			...
			if (item.type == 0) {
				if (item.target != this._owner) {
					...
					else if (startValue.b3) tweener.startValue.x = startValue.f1 * this._owner.width;
					if (!startValue.b2) tweener.startValue.y = item.target.y;
					else if (startValue.b3) tweener.startValue.y = startValue.f2 * this._owner.height;
	...
	callHook(item, tweenEnd) {
		...
		} else if (item.time >= this._startTime && item.hook != null) item.hook(item.label);
	}
	checkAllComplete() {
	...
	applyValue(item) {
		...
		switch (item.type) {
			case 0:
				...
				} else if (value.b1 && value.b2) item.target.setPosition(value.f1, value.f2);
			...
			case 13: break;
			case 6:
				let color = item.target.getProp(2);
				if (color instanceof Color) {
					let i = Math.floor(value.f1);
					color.r = i >> 16 & 255;
					color.g = i >> 8 & 255;
					color.b = i & 255;
					item.target.setProp(2, color);
				}
				break;
			case 7:
				if (value.frame >= 0) item.target.setProp(5, value.frame);
				item.target.setProp(4, value.playing);
				item.target.setProp(7, this._timeScale);
				break;
			case 8:
			...
			case 14:
				...
				break;
			case 15: item.target.icon = value.text;
		}
		item.target._gearLocked = false;
	}
	setup(buffer) {
						...
						for (var j = 0; j < pathLen; j++) {
							...
							switch (curveType) {
								...
								case 2:
									...
									break;
								default: pts.push(GPathPoint.newPoint(buffer.readFloat(), buffer.readFloat(), curveType));
							}
						}
	...
	decodeValue(item, buffer, value) {
		switch (item.type) {
			...
			case 13:
				...
				if (buffer.version >= 2 && item.type == 0) value.b3 = buffer.readBool();
				break;
			case 4:
			case 5:
				value.f1 = buffer.readFloat();
			...
			case 12:
				...
				break;
			case 14:
			case 15: value.text = buffer.readS();
		}
	}
};
...
var GComponent = class GComponent extends GObject {
	...
	dispose() {
		...
		for (i = 0; i < cnt; ++i) this._transitions[i].dispose();
		cnt = this._controllers.length;
		for (i = 0; i < cnt; ++i) this._controllers[i].dispose();
	...
	removeChildAt(index, dispose) {
		if (index >= 0 && index < this.numChildren) {
			...
			if (this._childrenRenderOrder == 2) this._partner.callLater(this.buildNativeDisplayList);
			if (dispose) child.dispose();
			else child.node.parent = null;
			this.setBoundsChangedFlag();
			return child;
		} else throw new Error("Invalid child index");
	}
	...
	getChild(name, classType) {
		var cnt = this._children.length;
		for (var i = 0; i < cnt; ++i) if (this._children[i].name == name) return this._children[i];
		return null;
	}
	...
	getChildById(id) {
		var cnt = this._children.length;
		for (var i = 0; i < cnt; ++i) if (this._children[i]._id == id) return this._children[i];
		return null;
	}
	getChildIndex(child) {
	...
	_setChildIndex(child, oldIndex, index) {
		...
		if (this._childrenRenderOrder == 0) child.node.setSiblingIndex(index);
		else if (this._childrenRenderOrder == 1) child.node.setSiblingIndex(cnt - index);
		else this._partner.callLater(this.buildNativeDisplayList);
		this.setBoundsChangedFlag();
		return index;
	}
	...
	removeController(c) {
		...
		for (var i = 0; i < length; i++) this._children[i].handleControllerChanged(c);
	}
	get controllers() {
	...
	buildNativeDisplayList(dt) {
		if (!isNaN(dt)) {
			GObject.cast(this.node).buildNativeDisplayList();
			return;
		}
		let cnt = this._children.length;
		if (cnt == 0) return;
	...
	applyAllControllers() {
		var cnt = this._controllers.length;
		for (var i = 0; i < cnt; ++i) this.applyController(this._controllers[i]);
	}
	...
	isChildInView(child) {
		if (this._rectMask) return child.x + child.width >= 0 && child.x <= this.width && child.y + child.height >= 0 && child.y <= this.height;
		else if (this._scrollPane) return this._scrollPane.isChildInView(child);
		else return true;
	}
	getFirstChildInView() {
		var cnt = this._children.length;
	...
	set apexIndex(value) {
		if (this._apexIndex != value) {
			...
			if (this._childrenRenderOrder == 2) this.buildNativeDisplayList();
		}
	}
	get mask() {
	...
	setMask(value, inverted) {
		...
		if (this._maskContent) {
			...
			if (!this._customMask) {
				...
				if (this._scrollPane) this._container.parent.parent = maskNode;
				else this._container.parent = maskNode;
			...
			if (this._node.activeInHierarchy) this.onMaskReady();
			else this.on(FGUIEvent.DISPLAY, this.onMaskReady, this);
			this.onMaskContentChanged();
			if (this._scrollPane) this._scrollPane.adjustMaskContainer();
			else this._container.setPosition(0, 0);
		} else if (this._customMask) {
			if (this._scrollPane) this._container.parent.parent = this._node;
			else this._container.parent = this._node;
			this._customMask.node.destroy();
			this._customMask = null;
			if (this._scrollPane) this._scrollPane.adjustMaskContainer();
			else this._container.setPosition(this._pivotCorrectX, this._pivotCorrectY);
		}
	}
	onMaskReady() {
		...
		if (this._maskContent instanceof GImage) {
			...
			if (this._maskContent.type == 2) this._customMask.type = Mask.Type.GRAPHICS_ELLIPSE;
			else this._customMask.type = Mask.Type.GRAPHICS_RECT;
		}
	...
	setupOverflow(overflow) {
		if (overflow == 1) this._rectMask = this._container.addComponent(Mask);
		if (!this._margin.isNone) this.handleSizeChanged();
	}
	handleAnchorChanged() {
		super.handleAnchorChanged();
		if (this._customMask) this._customMask.node.setPosition(this._pivotCorrectX, this._pivotCorrectY);
		else if (this._scrollPane) this._scrollPane.adjustMaskContainer();
		else this._container.setPosition(this._pivotCorrectX + this._alignOffset.x, this._pivotCorrectY - this._alignOffset.y);
	}
	handleSizeChanged() {
		super.handleSizeChanged();
		if (this._customMask) this._customMask.node.setPosition(this._pivotCorrectX, this._pivotCorrectY);
		else if (!this._scrollPane) this._container.setPosition(this._pivotCorrectX, this._pivotCorrectY);
		if (this._scrollPane) this._scrollPane.onOwnerSizeChanged();
		else this._containerUITrans.setContentSize(this.viewWidth, this.viewHeight);
	}
	handleGrayedChanged() {
		...
		var v = this.grayed;
		var cnt = this._children.length;
		for (var i = 0; i < cnt; ++i) this._children[i].grayed = v;
	}
	handleControllerChanged(c) {
	...
	_hitTest(pt, globalPt) {
		if (this._customMask) {
			...
			if (!(this._customMask.isHit(s_vec2$2) || false)) return null;
		}
		if (this.hitArea) {
	...
	refresh(dt) {
		if (!isNaN(dt)) {
			GObject.cast(this.node).refresh();
			return;
		}
		if (this._boundsChanged) {
			var len = this._children.length;
			if (len > 0) for (var i = 0; i < len; i++) this._children[i].ensureSizeCorrect();
			this.updateBounds();
		}
	}
	ensureBoundsCorrect() {
			...
			for (; i < cnt; i++) {
				...
				if (yValue < obj.y) {
					if (i == 0) {
						...
						if (yValue < prev.y + prev.actualHeight / 2) yValue = prev.y;
						else yValue = obj.y;
						break;
					}
				}
			...
			for (; i < cnt; i++) {
				...
				if (xValue < obj.x) {
					if (i == 0) {
						...
						prev = this._children[i - 1];
						if (xValue < prev.x + prev.actualWidth / 2) xValue = prev.x;
						else xValue = obj.x;
						break;
					}
				}
	...
	constructFromResource2(objectPool, poolIndex) {
		...
		if (maskId != -1) this.setMask(this.getChildAt(maskId), buffer.readBool());
		...
		} else if (i1 != 0 && i2 != -1) this.hitArea = new ChildHitArea(this.getChildAt(i2));
		buffer.seek(0, 5);
		...
		this.buildNativeDisplayList();
		this.setBoundsChangedFlag();
		if (contentItem.objectType != 9) this.constructExtension(buffer);
		this.onConstruct();
	}
...
var Window = class extends GComponent {
	...
	__uiLoadComplete() {
		var cnt = this._uiSources.length;
		for (var i = 0; i < cnt; i++) if (!this._uiSources[i].loaded) return;
		this._loading = false;
		this._init();
	}
	_init() {
...
var GRoot = class GRoot extends GComponent {
	...
	static create(root) {
		...
		if (root) root.addChild(GRoot._inst.node);
		else director.getScene().getChildByName("Canvas").addChild(GRoot._inst.node);
	...
	getTopWindow() {
		for (var i = this.numChildren - 1; i >= 0; i--) {
			var g = this.getChildAt(i);
			if (g instanceof Window) return g;
		}
		return null;
	}
	get modalLayer() {
	...
	getPopupPosition(popup, target, dir, result) {
		...
		pos.y += sizeH;
		if ((dir === void 0 || dir === 0) && pos.y + popup.height > this.height || dir === false || dir === 1) {
	...
	showPopup(popup, target, dir) {
		if (this._popupStack.length > 0) {
			...
			if (k != -1) for (var i = this._popupStack.length - 1; i >= k; i--) this.removeChild(this._popupStack.pop());
		}
		...
		if (target) {
			...
			while (p) {
				if (p.parent == this) {
					if (popup.sortingOrder < p.sortingOrder) popup.sortingOrder = p.sortingOrder;
					break;
				}
				p = p.parent;
			}
	...
	hidePopup(popup) {
		if (popup) {
			var k = this._popupStack.indexOf(popup);
			if (k != -1) for (var i = this._popupStack.length - 1; i >= k; i--) this.closePopup(this._popupStack.pop());
		} else {
	...
	playOneShotSound(clip, volumeScale) {
		if (!this.audioEngine) this.audioEngine = this.node.addComponent(AudioSourceComponent);
...
var GTextInput = class extends GTextField {
	...
	set promptText(val) {
		...
		if (defaultParser.lastSize) this._editBox.placeholderLabel.fontSize = parseInt(defaultParser.lastSize);
		else this._editBox.placeholderLabel.fontSize = this._fontSize;
	}
	get promptText() {
	...
	set align(value) {
		...
		if (this._editBox.placeholderLabel) this._editBox.placeholderLabel.horizontalAlign = value;
	}
	get verticalAlign() {
	...
	set verticalAlign(value) {
		...
		if (this._editBox.placeholderLabel) this._editBox.placeholderLabel.verticalAlign = value;
	}
	get singleLine() {
	...
	setup_beforeAdd(buffer, beginPos) {
		...
		else if (this._editBox.placeholderLabel) this._editBox.placeholderLabel.string = "";
		str = buffer.readS();
...
var MyEditBox = class extends EditBox {
	_init() {
		super._init();
...
var GLoader = class GLoader extends GObject {
	...
	*/
	...
	get bundle() {
		if (this._assetBundle) return this._assetBundle;
	...
	loadContent() {
		this.clearContent();
		if (!this._url) return;
		if (typeof this._url == "string" && this._url.startsWith("ui://")) this.loadFromPackage(this._url);
	...
	loadFromPackage(itemURL) {
		...
		if (contentItem.type == 0) {
			if (!contentItem.asset) this.setErrorState();
			else {
				this._content.spriteFrame = contentItem.asset;
				if (this._content.fillMethod == 0) {
					if (contentItem.scale9Grid) this._content.type = Sprite.Type.SLICED;
					else if (contentItem.scaleByTile) this._content.type = Sprite.Type.TILED;
					else this._content.type = Sprite.Type.SIMPLE;
				} else this._content.type = Sprite.Type.FILLED;
				this.updateLayout();
			}
	...
	loadExternal() {
		...
		let callback = (err, asset) => {
			...
			if (asset instanceof SpriteFrame) this.onExternalLoadSuccess(asset);
				...
				sf.texture = texture;
				this.onExternalLoadSuccess(sf);
			} else console.warn("GLoader:cant load", this.url);
		};
		if (typeof this._url == "string") {
			if (this._url.startsWith("http://") || this._url.startsWith("https://") || this._url.startsWith("/")) assetManager.loadRemote(this._url, callback);
				...
				if (this.bundle && assetManager.bundles.has(this.bundle)) bundle = assetManager.getBundle(this.bundle);
				bundle.load(this._url + "/spriteFrame", Asset, callback);
			}
		} else throw new Error("fgui底层未实现CCURL的非string资源加载！");
	}
	...
	setErrorState() {
		...
		if (this._errorSign == null) {
			if (UIConfig.loaderErrorSign != null) this._errorSign = GLoader._errorSignPool.getObject(UIConfig.loaderErrorSign);
		}
		if (this._errorSign) {
	...
	updateLayout() {
		...
		if (this._fill != 0) {
			...
			if (sx != 1 || sy != 1) {
				if (this._fill == 2) sx = sy;
				else if (this._fill == 3) sy = sx;
		...
		var nx, ny;
		if (this._align == 0) nx = 0;
		else if (this._align == 1) nx = Math.floor((this._width - cw) / 2);
		else nx = this._width - cw;
		if (this._verticalAlign == 0) ny = 0;
		else if (this._verticalAlign == 1) ny = Math.floor((this._height - ch) / 2);
		else ny = this._height - ch;
		ny = -ny;
	...
	getProp(index) {
		switch (index) {
			...
			case 4: return this.playing;
			case 5: return this.frame;
			case 7: return this._content.timeScale;
			default: return super.getProp(index);
		}
	}
	setProp(index, value) {
...
var GLoader3D = class extends GObject {
	...
	loadFromPackage(itemURL) {
		...
		if (this._contentItem) {
			...
			if (this._autoSize) this.setSize(this.sourceWidth, this.sourceHeight);
			if (this._contentItem.type == 9 || this._contentItem.type == 10) this._contentItem.owner.getItemAssetAsync(this._contentItem, this.onLoaded.bind(this));
		}
	}
	onLoaded(err, item) {
		...
		if (this._contentItem.type == 9) this.setSpine(this._contentItem.asset, this._contentItem.skeletonAnchor);
		else if (this._contentItem.type == 10) this.setDragonBones(this._contentItem.asset, this._contentItem.atlasAsset, this._contentItem.skeletonAnchor);
	}
	...
	onChange() {
		...
		if (this._contentItem.type == 9) this.onChangeSpine();
		if (this._contentItem.type == 10) this.onChangeDragonBones();
	}
	onChangeSpine() {
		...
		if (this._animationName) {
			...
			if (this._playing) this._content.paused = false;
			else {
		...
		if (((_this$_content$_skele = this._content["_skeleton"].skin) === null || _this$_content$_skele === void 0 ? void 0 : _this$_content$_skele.name) != skin) this._content.setSkin(skin);
	}
	onChangeDragonBones() {
		if (!(this._content instanceof dragonBones.ArmatureDisplay)) return;
		if (this._animationName) {
			if (this._playing) this._content.playAnimation(this._animationName, this._loop ? 0 : 1);
			else this._content.armature().animation.gotoAndStopByFrame(this._animationName, this._frame);
		} else this._content.armature().animation.reset();
	}
	loadExternal() {
		if (this._url.startsWith("http://") || this._url.startsWith("https://") || this._url.startsWith("/")) assetManager.loadRemote(this._url, sp.SkeletonData, this.onLoaded2.bind(this));
		else resources.load(this._url, sp.SkeletonData, this.onLoaded2.bind(this));
	}
	onLoaded2(err, asset) {
...
var GLabel = class extends GComponent {
	...
	get editable() {
		if (this._titleObject && this._titleObject instanceof GTextInput) return this._titleObject.editable;
		else return false;
	}
	getTextField() {
	...
	getProp(index) {
		switch (index) {
			case 2: return this.titleColor;
			case 3:
				var tf = this.getTextField();
				if (tf) return tf.strokeColor;
				else return 0;
			case 8: return this.titleFontSize;
			default: return super.getProp(index);
		}
	}
	setProp(index, value) {
...
var GButton = class GButton extends GComponent {
	...
	set selected(val) {
		...
		if (this._selected != val) {
			...
			if (this._relatedController && this._parent && !this._parent._buildingDisplayList) {
				...
				} else if (this._mode == 1 && this._relatedController.selectedPageId == this._relatedPageId) this._relatedController.oppositePageId = this._relatedPageId;
			}
		}
	}
	get selected() {
	...
	set mode(value) {
		if (this._mode != value) {
			if (value == 0) this.selected = false;
			this._mode = value;
		}
	}
		...
		if (this._downEffect == 1) {
			...
			if (val == GButton.DOWN || val == GButton.SELECTED_OVER || val == GButton.SELECTED_DISABLED) {
				...
				for (var i = 0; i < cnt; i++) {
					...
					if (!(obj instanceof GTextField)) obj.setProp(2, this._downColor);
				}
			} else for (var i = 0; i < cnt; i++) {
				var obj = this.getChildAt(i);
				if (!(obj instanceof GTextField)) obj.setProp(2, Color.WHITE);
			}
	...
	setCurrentState() {
		...
		} else if (this._selected) this.setState(this._over ? GButton.SELECTED_OVER : GButton.DOWN);
	...
	handleGrayedChanged() {
		if (this._buttonController && this._buttonController.hasPage(GButton.DISABLED)) {
			if (this.grayed) {
				if (this._selected && this._buttonController.hasPage(GButton.SELECTED_DISABLED)) this.setState(GButton.SELECTED_DISABLED);
				else this.setState(GButton.DISABLED);
	...
	constructExtension(buffer) {
		...
		if (this._mode == 0) this.setState(GButton.UP);
	...
	onTouchBegin_1(evt) {
		...
		evt.captureTouch();
		if (this._mode == 0) {
			if (this.grayed && this._buttonController && this._buttonController.hasPage(GButton.DISABLED)) this.setState(GButton.SELECTED_DISABLED);
			else this.setState(GButton.DOWN);
		}
		if (this._linkedPopup) {
			if (this._linkedPopup instanceof Window) this._linkedPopup.toggleStatus();
			else GRoot.inst.togglePopup(this._linkedPopup, this);
		}
	}
	onTouchEnd_1(evt) {
		if (evt.button != EventMouse.BUTTON_LEFT) return;
		if (this._down) {
			...
			if (this._mode == 0) {
				if (this.grayed && this._buttonController && this._buttonController.hasPage(GButton.DISABLED)) this.setState(GButton.DISABLED);
				else if (this._over) this.setState(GButton.OVER);
				else this.setState(GButton.UP);
			} else if (!this._over && this._buttonController != null && (this._buttonController.selectedPage == GButton.OVER || this._buttonController.selectedPage == GButton.SELECTED_OVER)) this.setCurrentState();
		}
	}
	onClick_1() {
		if (this._sound) {
		...
		} else if (this._relatedController) this._relatedController.selectedPageId = this._relatedPageId;
	}
};
GButton.UP = "up";
GButton.DOWN = "down";
...
var GList = class extends GComponent {
	...
	returnToPool(obj) {
		if (this.backToPool) this.backToPool(obj);
	...
	removeChildAt(index, dispose) {
		...
		if (!dispose) child.off(FGUIEvent.CLICK, this.onClickItem, this);
		return child;
	}
	...
	set selectedIndex(value) {
		if (value >= 0 && value < this.numItems) {
			if (this._selectionMode != 0) this.clearSelection();
	...
	addSelection(index, scrollItToView) {
		if (this._selectionMode == 3) return;
	...
	handleArrowKey(dir) {
		...
		switch (dir) {
			...
			case 5:
				...
				break;
			case 7: if (this._layout == 1 || this._layout == 2 || this._layout == 4) {
				index--;
				if (index >= 0) {
					this.clearSelection();
					this.addSelection(index, true);
	...
	setSelectionOnEvent(item, evt) {
		if (!(item instanceof GButton) || this._selectionMode == 3) return;
		...
		if (this._selectionMode == 0) {
			...
			if (!item.selected) {
				if (this._lastSelectedIndex != -1) {
					...
					var i;
					if (this._virtual) for (i = min; i <= max; i++) {
						var ii = this._virtualItems[i];
		...
		} else if (evt.isCtrlDown || this._selectionMode == 2) item.selected = !item.selected;
	...
	resizeToFit(itemCount = Number.POSITIVE_INFINITY, minSize = 0) {
		...
		if (this._virtual) {
			...
			if (this._layout == 0 || this._layout == 2) this.viewHeight = lineCount * this._itemSize.height + Math.max(0, lineCount - 1) * this._lineGap;
			else this.viewWidth = lineCount * this._itemSize.width + Math.max(0, lineCount - 1) * this._columnGap;
		} else if (itemCount == 0) {
			if (this._layout == 0 || this._layout == 2) this.viewHeight = minSize;
	...
	scrollToView(index, ani, setFirst) {
		if (this._virtual) {
			...
			if (obj) {
				...
				else if (this.parent && this.parent.scrollPane) this.parent.scrollPane.scrollToView(obj, ani, setFirst);
			}
		}
	}
	getFirstChildInView() {
	...
	childIndexToItemIndex(index) {
		...
		if (this._layout == 4) {
			for (var i = this._firstIndex; i < this._realNumItems; i++) if (this._virtualItems[i].obj) {
				index--;
				if (index < 0) return i;
			}
			return index;
	...
	itemIndexToChildIndex(index) {
		if (!this._virtual) return index;
		if (this._layout == 4) return this.getChildIndex(this._virtualItems[index].obj);
		else {
	...
	_setVirtual(loop) {
		if (!this._virtual) {
			...
			if (loop) {
				if (this._layout == 2 || this._layout == 3) throw new Error("Loop list is not supported for FlowHorizontal or FlowVertical layout!");
			...
			if (this._itemSize == null) {
				...
				if (!obj) throw new Error("Virtual List must have a default list item resource.");
				else {
	...
	set numItems(value) {
		if (this._virtual) {
			...
			if (this._realNumItems > oldCount) for (i = oldCount; i < this._realNumItems; i++) {
				var ii = {
			...
			else for (i = this._realNumItems; i < oldCount; i++) this._virtualItems[i].selected = false;
			if (this._virtualListChanged != 0) this._partner.unschedule(this._refreshVirtualList);
			this._refreshVirtualList();
		} else {
			var cnt = this._children.length;
			if (value > cnt) for (var i = cnt; i < value; i++) if (this.itemProvider == null) this.addItemFromPool();
			else this.addItemFromPool(this.itemProvider(i));
			else this.removeChildrenToPool(value, cnt);
			if (this.itemRenderer != null) for (i = 0; i < value; i++) this.itemRenderer(i, this.getChildAt(i));
		}
	}
	refreshVirtualList() {
	...
	_refreshVirtualList(dt) {
		if (!isNaN(dt)) {
			GObject.cast(this.node)._refreshVirtualList();
			return;
		}
		...
		if (layoutChanged) {
			if (this._layout == 0 || this._layout == 1) this._curLineItemCount = 1;
			else if (this._layout == 2) {
	...
	handleScroll1(forceUpdate) {
		...
		while (curIndex < this._realNumItems && (end || curY < max)) {
			...
			if (!ii.obj) {
				if (forward) for (j = reuseIndex; j >= oldFirstIndex; j--) {
				...
				else for (j = reuseIndex; j <= lastIndex; j++) {
					ii2 = this._virtualItems[j];
					if (ii2.obj && ii2.updateFlag != this.itemInfoVer && ii2.obj.resourceURL == url) {
						if (ii2.obj instanceof GButton) ii2.selected = ii2.obj.selected;
						ii.obj = ii2.obj;
				...
				if (ii.obj) this.setChildIndex(ii.obj, forward ? curIndex - newFirstIndex : this.numChildren);
				else {
			...
			if (needRender) {
				if (this._autoResizeItem && (this._layout == 0 || this._columnCount > 0)) ii.obj.setSize(partSize, ii.obj.height, true);
				this.itemRenderer(curIndex % this._numItems, ii.obj);
				if (curIndex % this._curLineItemCount == 0) {
					deltaSize += Math.ceil(ii.obj.height) - ii.height;
					if (curIndex == newFirstIndex && oldFirstIndex > newFirstIndex) firstItemDeltaSize = Math.ceil(ii.obj.height) - ii.height;
				}
		...
		if (curIndex > 0 && this.numChildren > 0 && this._container.position.y <= 0 && this.getChildAt(0).y > -this._container.position.y) return true;
		else return false;
	}
	handleScroll2(forceUpdate) {
		...
		while (curIndex < this._realNumItems && (end || curX < max)) {
			...
			if (needRender) {
				if (this._autoResizeItem && (this._layout == 1 || this._lineCount > 0)) ii.obj.setSize(ii.obj.width, partSize, true);
				this.itemRenderer(curIndex % this._numItems, ii.obj);
				if (curIndex % this._curLineItemCount == 0) {
					deltaSize += Math.ceil(ii.obj.width) - ii.width;
					if (curIndex == newFirstIndex && oldFirstIndex > newFirstIndex) firstItemDeltaSize = Math.ceil(ii.obj.width) - ii.width;
				}
				ii.width = Math.ceil(ii.obj.width);
				ii.height = Math.ceil(ii.obj.height);
			}
		...
		if (curIndex > 0 && this.numChildren > 0 && this._container.position.x <= 0 && this.getChildAt(0).x > -this._container.position.x) return true;
		else return false;
	}
	handleScroll3(forceUpdate) {
		...
		var viewWidth = this.viewWidth;
		var startIndex = Math.floor(newFirstIndex / pageSize) * pageSize;
	...
	handleAlign(contentWidth, contentHeight) {
		...
		if (contentHeight < this.viewHeight) {
			if (this._verticalAlign == 1) newOffsetY = Math.floor((this.viewHeight - contentHeight) / 2);
			else if (this._verticalAlign == 2) newOffsetY = this.viewHeight - contentHeight;
		}
		if (contentWidth < this.viewWidth) {
			if (this._align == 1) newOffsetX = Math.floor((this.viewWidth - contentWidth) / 2);
			else if (this._align == 2) newOffsetX = this.viewWidth - contentWidth;
		}
	...
	updateBounds() {
		...
		if (this._layout == 0) {
			...
			if (this._autoResizeItem && this._columnCount > 0) for (i = 0; i < cnt; i++) {
				child = this.getChildAt(i);
				if (this.foldInvisibleItems && !child.visible) continue;
	...
	updateWithPercent(percent, manual) {
		...
		if (this._titleObject) switch (this._titleType) {
			...
			case 2:
				...
				break;
			case 3: this._titleObject.text = "" + this._max;
		}
	...
	update(newValue) {
		...
		if (this._titleObject) switch (this._titleType) {
			...
			case 2:
				...
				break;
			case 3: this._titleObject.text = "" + Math.floor(this._max);
		}
		var fullWidth = this.width - this._barMaxWidthDelta;
		var fullHeight = this.height - this._barMaxHeightDelta;
		if (!this._reverse) {
		...
		if (this._aniObject) this._aniObject.setProp(5, Math.floor(percent * 100));
	}
	setFillAmount(bar, percent) {
		if ((bar instanceof GImage || bar instanceof GLoader) && bar.fillMethod != 0) {
...
var GScrollBar = class extends GComponent {
	...
	constructExtension(buffer) {
		...
		if (this._arrowButton1) this._arrowButton1.on(FGUIEvent.TOUCH_BEGIN, this.onClickArrow1, this);
		if (this._arrowButton2) this._arrowButton2.on(FGUIEvent.TOUCH_BEGIN, this.onClickArrow2, this);
	...
	onBarTouchBegin(evt) {
		...
		} else if (pt.x < 0) this._target.scrollLeft(4);
...
var GTreeNode = class {
	...
	addChildAt(child, index) {
		...
		var numChildren = this._children.length;
		if (index >= 0 && index <= numChildren) {
			if (child._parent == this) this.setChildIndex(child, index);
			else {
				if (child._parent) child._parent.removeChild(child);
				if (index == this._children.length) this._children.push(child);
				else this._children.splice(index, 0, child);
				child._parent = this;
	...
	removeChild(child) {
		var childIndex = this._children.indexOf(child);
		if (childIndex != -1) this.removeChildAt(childIndex);
		return child;
	}
	removeChildAt(index) {
		if (index >= 0 && index < this.numChildren) {
...
var GTree = class extends GList {
	...
	createCell(node) {
		...
		var cc = child.getController("expanded");
		if (cc) {
		...
		if (node.isFolder) node._cell.on(FGUIEvent.TOUCH_BEGIN, this.__cellMouseDown, this);
	...
	getInsertIndexForNode(node) {
		...
		for (var i = insertIndex; i < cnt; i++) {
			if (this.getChildAt(i)._treeNode.level <= myLevel) break;
			insertIndex++;
		}
		return insertIndex;
	}
	...
	_afterMoved(node) {
		...
		var obj;
		if (insertIndex < startIndex) for (i = 0; i < cnt; i++) {
	...
	getFolderEndIndex(startIndex, level) {
		var cnt = this.numChildren;
		for (var i = startIndex + 1; i < cnt; i++) if (this.getChildAt(i)._treeNode.level <= level) return i;
		return cnt;
	}
	...
	readItems(buffer) {
		...
		for (i = 0; i < cnt; i++) {
			...
			else if (level > prevLevel) lastNode.addChild(node);
...
var PopupMenu = class {
	...
	setItemCheckable(name, checkable) {
		var c = this._list.getChild(name).getController("checked");
		if (c) {
			if (checkable) {
...
var UIObjectFactory = class UIObjectFactory {
	...
	static newObject(type, userClass) {
		...
		if (typeof type === "number") switch (type) {
			case 0: return new GImage();
			case 1: return new GMovieClip();
			case 9: return new GComponent();
			case 6: return new GTextField();
			case 7: return new GRichTextField();
			case 8: return new GTextInput();
			case 5: return new GGroup();
			case 10: return new GList();
			case 3: return new GGraph();
			case 4: if (UIObjectFactory.loaderType) return new UIObjectFactory.loaderType();
			else return new GLoader();
			case 12: return new GButton();
			case 11: return new GLabel();
			case 14: return new GProgressBar();
			case 15: return new GSlider();
			case 16: return new GScrollBar();
			case 13: return new GComboBox();
			case 17: return new GTree();
			case 18: return new GLoader3D();
			default: return null;
		}
		else {
			if (type.type == 3) {
...
var AsyncOperationRunner = class extends Component {
	constructor() {
		super();
	...
	collectComponentChildren(item) {
		...
		for (i = 0; i < dcnt; i++) {
			...
			if (src != null) {
				...
				if (pi && pi.type == 3) di.childCount = this.collectComponentChildren(pi);
			} else {
				di = { type };
				if (type == 10) di.listItemCount = this.collectListChildren(buffer);
			}
			this._itemList.push(di);
			buffer.position = curPos + dataLen;
		}
		...
		for (i = 0; i < itemCount; i++) {
			...
			if (url) {
				...
				if (pi) {
					...
					if (pi.type == 3) di.childCount = this.collectComponentChildren(pi);
					this._itemList.push(di);
					listItemCount++;
				}
			}
		...
		while (this._index < totalItems) {
			...
			if (di.pi) {
				...
				if (di.type == 10 && di.listItemCount > 0) {
					...
					for (k = 0; k < di.listItemCount; k++) obj.itemPool.returnObject(this._objectPool[k + poolStart]);
					this._objectPool.splice(poolStart, di.listItemCount);
				}
			}
			this._index++;
			if (this._index % 5 == 0 && game.totalTime / 1e3 - t >= frameTime) return;
		}
...
var SmartLoader = class extends GLoader {
	...
	loadExternal() {
		...
		this.__resRequest = Res.create(this.url, this.refKey, void 0, (err) => {
			if (err) {
				this.__resRequest.dispose();
				this.__resRequest = null;
			...
			} else this.onExternalLoadSuccess(ref.content);
...
//#endregion
export { AlignType, AsyncOperation, AutoSizeType, BlendMode, ButtonMode, ByteBuffer, ChildrenRenderOrder, Controller, DragDropManager, EaseType, FGUIEvent, FillMethod, FillOrigin, FlipType, GButton, GComboBox, GComponent, GGraph, GGroup, GImage, GLabel, GList, GLoader, GLoader3D, GMovieClip, GObject, GObjectPool, GProgressBar, GRichTextField, GRoot, GScrollBar, GSlider, GTextField, GTextInput, GTree, GTreeNode, GTween, GTweener, GearAnimation, GearBase, GearColor, GearDisplay, GearDisplay2, GearFontSize, GearIcon, GearLook, GearSize, GearText, GearXY, GroupLayoutType, Image$1 as Image, ListLayoutType, ListSelectionMode, LoaderFillType, MovieClip, ObjectPropID, ObjectType, OverflowType, PackageItem, PackageItemType, PopupDirection, PopupMenu, ProgressTitleType, RelationType, ScrollBarDisplayType, ScrollPane, ScrollType, SmartLoader, TooltipManager, Transition, TranslationHelper, UBBParser, UIConfig, UIObjectFactory, UIPackage, VertAlignType, Window, registerFont };
